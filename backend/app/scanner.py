...
RULES: list[Rule] = [
    ...
    Rule("student_id", "学号 / 工号", "medium", re.compile(r"(?:学号|工号)\s*[:=：]?\s*\d{6,14}")),
    ...
    Rule("api_key", "API Key / Token", "critical", re.compile(r"(?i)(?:\b(?:api[_-]?key|access[_-]?token|secret[_-]?key|bearer)\b\s*(?:[:=：]|为)?\s*['\"]?[A-Za-z0-9._\-]{12,}|sk-[A-Za-z0-9_-]{12,})")),
